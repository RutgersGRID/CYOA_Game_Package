...
public class StorySheetReader : MonoBehaviour
{
    ...
    private const string SHEET_URL = "https://sheets.googleapis.com/v4/spreadsheets/1Jwj58ptzQYkZvjsmVyzzCos5BwxwezuHiDb5BF3uXE0/values:batchGet?ranges=Sheet1&key=AIzaSyDxlgY5nx2_JX89Grs3KZ7cnxlpRO2Nedg";
    private void Start()
    {
    ...
    IEnumerator ObtainSheetData()
{
    ...
    else
    {
        ...
        for (int i = 1; i < valuesArray.Count; i++)  // Start from 1 to skip the first row
        {
            ...
            // You need to load sprites from these paths or URLs later
            string leftSideSpeakerPath = item[3].Value;
            string rightSideSpeakerPath = item[4].Value;
            string propPath = item[5].Value;
            string backgroundPath = item[6].Value;
            ...
            dialogues.Add(CreateDialogueSO(ID, Speaker, Line, Checkpoint, Type, GoToID, Effect, A1Answer, GoToIDA1, EffectA1, A2Answer, GoToIDA2, EffectA2, A3Answer, GoToIDA3, EffectA3));
        }
    }
}
}
